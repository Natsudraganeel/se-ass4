...
void updateinventory(){ 
 ...
 scanf("%d", &val); 
 switch(val){ 
 case 1: printf("Enter the product name: ");  fflush(stdin); 
 scanf("%s",&iv.productname);  break; 
 case 2: printf("Enter product quantity: ");  fflush(stdin); 
 scanf("%d", &iv.quantity); 
 break; 
 case 3: printf("Enter the product price: ");
 fflush(stdin); 
 scanf("%f", &iv.price);  break; 
 default: printf("invalid input\n");  } 
 fseek(fp, -sizeof(iv),1); 
 fwrite(&iv, sizeof(iv), 1, fp);  fclose(fp); 
 break; 
 } 
 } 
 if(f==1){ 
...
void del(int id){ 
 int f=0;
 FILE *ft; 
 fp=fopen("product.txt","rb");  ft=fopen("temp.txt","wb"); 
 while(fread(&iv, sizeof(iv), 1,fp) ==1){  if(id==iv.id){ 
 f=1; 
 } 
 else{ 
...
void buy(){ 
 int id,f=0,quant; 
 system("cls"); 
 printf("<== Buy products ==>\n\n");  printf("enter the product id to buy: ");  scanf("%d",&id); 
 printf("enter the quantity of the product: ");  scanf("%d",&quant); 
 FILE *ft; 
 float p; 
 fp=fopen("product.txt", "rb+"); 
 while(fread(&iv, sizeof(iv), 1,fp)==1){  if(id==iv.id){ 
...
int main(){ 
 ...
 printf("0. Exit\n"); 
 printf("-->Enter your choice: ");  scanf("%d",&val); 
 switch(val){ 
